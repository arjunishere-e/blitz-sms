...
function createPointerData(
  ...
  if (!pointerMap.has(options.domElement)) {
    ...
    if (!globalPointerActive) {
      ...
      // Touch events disabled - they block all mobile taps with preventDefault()
      // document.body.addEventListener(
      //   "touchstart",
  ...
  setColors(colors: number[]) {
    if (Array.isArray(colors) && colors.length > 1) {
      ...
      for (let idx = 0; idx < this.count; idx++) {
        ...
        const colorIdx = idx % colorObjects.length;
        this.setColorAt(idx, colorObjects[colorIdx]);
        if (idx === 0) {
          this.light!.color.copy(colorObjects[colorIdx]);
        }
      }
...
const Ballpit: React.FC<BallpitProps> = ({
  ...
  }, [followCursor, props.count, props.minSize, props.maxSize]);
